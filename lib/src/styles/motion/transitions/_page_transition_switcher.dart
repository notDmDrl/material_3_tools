...
class _ChildEntry {
  ...
  @override
  String toString() =>
      'PageTransitionSwitcherEntry#${shortHash(this)}($widgetChild)';
}
...
class _PageTransitionSwitcherState extends State<PageTransitionSwitcher>
  ...
  _ChildEntry _newEntry({
    ...
    secondaryController.addStatusListener((status) {
    ...
    primaryController.addStatusListener((status) {
  ...
  Widget build(BuildContext context) => widget.layoutBuilder(
    _activeEntries.map<Widget>((entry) => entry.transition).toList(),
  );
}
